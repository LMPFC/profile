...
% Calculate the number of cases that have been specified
totalNumberOfPossibleCases = size(rResReq,2)*size(thetaResReq,2)*...
    size(lengthResReq,2)*size(pipeWallReq,2)*size(airGapReq,2)*...
    size(lPipeReq,2)*size(magORReq,2)*size(magIRReq,2)*size(magHReq,2);
...
for aIter = 1:numOfaCases
    ...
    for caseIter = casesOfInterest
        ...
        Bx = importdata(['flowProfileData/',dirList(caseIter+2).name,...
            '/flowProfile_PipeBFieldX.csv']);
        By = importdata(['flowProfileData/',dirList(caseIter+2).name,...
            '/flowProfile_PipeBFieldY.csv']);
        Bz = importdata(['flowProfileData/',dirList(caseIter+2).name,...
            '/flowProfile_PipeBFieldZ.csv']);
        
        xMesh = importdata(['flowProfileData/',dirList(caseIter+2).name,...
            '/flowProfile_PipeMeshX.csv']);
        yMesh = importdata(['flowProfileData/',dirList(caseIter+2).name,...
            '/flowProfile_PipeMeshY.csv']);
        zMesh = importdata(['flowProfileData/',dirList(caseIter+2).name,...
